...
class ModelTimeSeriesEditor:
    ...
    def refresh_view(self):
        ...
        try:
            ...
            # The path should end with "fileName" and the parent should contain "mandatory"
            if path[-1] != "fileName":
                return False
            
            # Get the parent path (the time series object)
            parent_path = path[:-1]
            
            # Check if this time series is marked as mandatory
            mandatory_value = self.get_value_at_path(self.data, parent_path + ["mandatory"])
            
            # A time series is validated if it's mandatory, regardless of generated status
            # mandatory=true, generated=false -> input file that must exist
            # mandatory=true, generated=true -> output file that must be generated but is required
            # mandatory=false, generated=true -> optional output file
            # mandatory=false, generated=false -> optional input file
